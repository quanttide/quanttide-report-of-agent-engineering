...
整理软件配置文件是一类高精度工作：字段值必须精确匹配官方文档、JSON 语法和语义都不能出错、多个模型参数之间存在联动约束（如上下文窗口、最大输出 token、max_completion_tokens 三者关系）。这类任务天然考验 AI 的细心和耐心——容易遗漏模型、容易填错限额、容易忽略格式细节（如 URL 前导空格）。

选择配置修复作为测评场景，是因为它能直接反映模型在「精确读取文档 → 对比现有代码 → 做正确修改」这一完整链条上的能力差异，比开放式问答更能区分模型的实际工程可用性。
...
GLM 在整个配置修复任务中表现稳定，能正确读取文档、定位缺陷、生成符合格式的配置，整个过程无需人工干预。但 3.77 元的单次任务成本是 DeepSeek 的几十倍，对于日常编码场景难以承受。如果智谱后续降价到合理区间，其稳定性表现值得作为主力模型使用。
